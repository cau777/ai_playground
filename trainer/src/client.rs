...
impl ServerClient {
    pub fn new(config: &EnvConfig) -> Self {
        Self {
            ...
            //     .build()
            //     .unwrap(),
            blocking_client: blocking::Client::builder()
                .danger_accept_invalid_certs(true)
    ...
    pub fn submit(&self, storage: &GenericStorage, loss: f64, name: &str) {
        ...
        let response = self.blocking_client.post(self.create_url_with_name(name, "trainable"))
            .body(bytes)
            .send()
            .unwrap();
    ...
    pub fn load_model_config(&self, name: &str) -> Result<ModelXmlConfig, Box<dyn Error>> {
        let response = self.blocking_client.get(self.create_url_with_name(name, "config")).send()?;
        response.error_for_status_ref()?;
        Ok(load_model_xml(&response.bytes()?)?)
    }

    pub fn get_trainable(&self, name: &str) -> reqwest::Result<blocking::Response> {
        self.blocking_client.get(self.create_url_with_name(name, "trainable")).send()
    }
