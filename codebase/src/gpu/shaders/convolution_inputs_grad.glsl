...
//layout(constant_id = 10) const uint grad_max_h = 0;
//layout(constant_id = 11) const uint grad_max_w = 0;

void main() {
//    result_data.data[gl_GlobalInvocationID.x] = gl_GlobalInvocationID.x;
...
//    const uint index = b_and_in_c*h*w + h*w + w;
//    const float a = kernel_data.data[in_channels] + grad_data.data[in_channels];
        ...
        const uint max_h = min(kernel_size, padded_h + 1);
        const uint min_h = max(padded_h / stride + 1, grad_height) - grad_height;// *
        ...
        while (kernel_h < max_h) {
            ...
            uint max_w = min(kernel_size, padded_w + 1);
            uint min_w = max(padded_w / stride + 1, grad_width) - grad_width; // *
    ...
    //    data.result[gl_GlobalInvocationID.x] = result;
    ...
    //                result += grad[(b, out_c, grad_h, grad_w)]
    //                  * kernel[(out_c, in_c, kernel_h, kernel_w)]
    //            }
    //
    //            kernel_w += stride;
    //        }
